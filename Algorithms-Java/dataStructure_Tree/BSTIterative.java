...
public class BSTIterative {
	...
	public static void main(String[] args) {
		...
		/*
		 * Will print following order 3 9 30 40
		 */
		tree.inorder();
	}

	/**
	 * A method to insert a new value in BST. If the given value is already present
	 * in BST the insertion is ignored.
	 *
	...
	public void add(int data) {
		...
		int rightOrLeft = -1;
		/*
		 * Finds the proper place this node can be placed in according to rules of BST.
		 */
		while (temp != null) {
			if (temp.data > data) {
				parent = temp;
		...
		/*
		 * Creates a newNode with the value passed Since this data doesn't already
		 * exists
		 */
	...
	public void remove(int data) {
		...
		int rightOrLeft = -1;
		/*
		 * Find the parent of the node and node itself That is to be deleted. parent
		 * variable store parent temp stores node itself. rightOrLeft use to keep track
		 * weather child is left or right subtree
		 */
		while (temp != null) {
			if (temp.data == data) {
				break;
		...
		/*
		 * If temp is null than node with given value is not present in our tree.
		 */
		if (temp != null) {
			...
			if (temp.right == null && temp.left == null) { // Leaf node Case
				...
				/*
				 * If both left and right child are present we replace this nodes data with
				 * leftmost node's data in its right subtree to maintain the balance of BST. And
